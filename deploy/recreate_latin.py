#!/usr/bin/env python3
"""Пересоздание test_db и копирование с обработкой кодировки."""
...
def main():
    ...
    try:
        ...
        print("[2/3] Создаю test_db...")
        # Создаём LATIN1 (локаль en_US)
        rc, out = run(ssh, "sudo -u postgres createdb test_db")
        print(f"    {'OK' if rc == 0 else out}")
        ...
        print("[3/3] Копирую данные через pg_dump с --data-only...")
        # Используем pg_dump с DATA_ONLY и encoding
        rc, out = run(ssh, "sudo -u postgres pg_dump --data-only --encoding=UTF8 test | sudo -u postgres psql -d test_db 2>&1")
        print(f"    Данные: {'OK' if rc == 0 else out[:200]}")
