#!/usr/bin/env python3
"""Копирование данных с правильной кодировкой."""
...
# Подключение к test_db (приёмник) с UTF8
dst = psycopg2.connect(dbname="test_db", user="bot_secretar", password=",jnctrhtnfhm2024", host="localhost")
dst.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
dst_cursor = dst.cursor()

# Устанавливаем клиентскую кодировку
dst_cursor.execute("SET client_encoding TO 'UTF8'")
...
src_cursor.execute("SELECT id, name, base_url, updated_at FROM organizers")
...
print(f"Organizers inserted: {len(orgs)}")
...
print(f"Locations inserted: {len(locs)}")
...
# Проверка названий
dst_cursor.execute("SELECT name FROM organizers LIMIT 5")
print("\\nOrganizer names:")
